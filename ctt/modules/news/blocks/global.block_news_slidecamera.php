...
if (! nv_function_exists('nv_block_news_slidecamera')) {
    function nv_block_config_news_slidecamera($module, $data_block, $lang_block)
    {
    ...
    function nv_block_config_news_slidecamera_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_block_news_slidecamera($block_config)
    {
        ...
        if (! empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_slidecamera.tpl')) {
                $block_theme = $global_config['module_theme'];
            } else {
                $block_theme = 'default';
            }
            $xtpl = new XTemplate('block_slidecamera.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news');
            ...
            foreach ($list as $l) {
                ...
                if ($l['homeimgthumb'] == 1) {
					...
					$l['fileimg'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $l['homeimgfile'];
					...
					$l['fileimg'] = $l['homeimgfile'];
                } elseif (! empty($show_no_image)) {
                    $l['thumb'] = NV_BASE_SITEURL . $show_no_image;
					$l['fileimg'] = NV_BASE_SITEURL . $show_no_image;
                } else {
                    $l['thumb'] = '';
					$l['fileimg'] = '';
                }
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_block_news_slidecamera($block_config);
    }
}
