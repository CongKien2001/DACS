...
else
{
	if ( $nv_Request->isset_request( 'add', 'get' ) or $nv_Request->isset_request( 'edit, id', 'get' ) )
	{
	    ...
	    if ( $nv_Request->isset_request( 'save', 'post' ) )
	    {
			...
			$post['delegate'] = $nv_Request->get_title( 'delegate', 'post', '', 1 );
			$post['delegate'] = nv_nl2br( $post['delegate'], "<br />" );
			
			$post['execution'] = $nv_Request->get_title( 'execution', 'post', '', 1 );
			$post['execution'] = nv_nl2br( $post['execution'], "<br />" );
			
			$post['ingredient'] = $nv_Request->get_title( 'ingredient', 'post', '', 1 );
			$post['ingredient'] = nv_nl2br( $post['ingredient'], "<br />" );
			
			$post['leadtime'] = $nv_Request->get_title( 'leadtime', 'post', '', 1 );
			$post['leadtime'] = nv_nl2br( $post['leadtime'], "<br />" );
			
			$post['subjects'] = $nv_Request->get_title( 'subjects', 'post', '', 1 );
			$post['subjects'] = nv_nl2br( $post['subjects'], "<br />" );
			
			$post['result'] = $nv_Request->get_title( 'result', 'post', '', 1 );
			$post['result'] = nv_nl2br( $post['result'], "<br />" );
			
			$post['fees'] = $nv_Request->get_title( 'fees', 'post', '', 1 );
			$post['fees'] = nv_nl2br( $post['fees'], "<br />" );
			
			$post['declarationform'] = $nv_Request->get_title( 'declarationform', 'post', '', 1 );
			$post['declarationform'] = nv_nl2br( $post['declarationform'], "<br />" );
			
			$post['legalbasis'] = $nv_Request->get_title( 'legalbasis', 'post', '', 1 );
			$post['legalbasis'] = nv_nl2br( $post['legalbasis'], "<br />" );
	        ...
	        if ( isset( $post['id'] ) )
	        {
	            $query = "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_row SET
					...
					delegate=" . $db->quote( $post['delegate'] ) . ",
					execution=" . $db->quote( $post['execution'] ) . ",
					ingredient=" . $db->quote( $post['ingredient'] ) . ",
					leadtime=" . $db->quote( $post['leadtime'] ) . ",
					subjects=" . $db->quote( $post['subjects'] ) . ",
					result=" . $db->quote( $post['result'] ) . ",
					fees=" . $db->quote( $post['fees'] ) . ",
					declarationform=" . $db->quote( $post['declarationform'] ) . ",
					legalbasis=" . $db->quote( $post['legalbasis'] ) . ",
	        ...
	        else
	        {
	            $query = "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_row VALUES
					...
					" . $db->quote( $post['delegate'] ) . ",
					" . $db->quote( $post['execution'] ) . ",
					" . $db->quote( $post['ingredient'] ) . ",
					" . $db->quote( $post['leadtime'] ) . ",
					" . $db->quote( $post['subjects'] ) . ",
					" . $db->quote( $post['result'] ) . ",
					" . $db->quote( $post['fees'] ) . ",
					" . $db->quote( $post['declarationform'] ) . ",
					" . $db->quote( $post['legalbasis'] ) . ",
		...
		if ( ! empty( $post['delegate'] ) ) $post['delegate'] = nv_htmlspecialchars( $post['delegate'] );
		if ( ! empty( $post['execution'] ) ) $post['execution'] = nv_htmlspecialchars( $post['execution'] );
		if ( ! empty( $post['ingredient'] ) ) $post['ingredient'] = nv_htmlspecialchars( $post['ingredient'] );
		if ( ! empty( $post['leadtime'] ) ) $post['leadtime'] = nv_htmlspecialchars( $post['leadtime'] );
		if ( ! empty( $post['subjects'] ) ) $post['subjects'] = nv_htmlspecialchars( $post['subjects'] );
		if ( ! empty( $post['result'] ) ) $post['result'] = nv_htmlspecialchars( $post['result'] );
		if ( ! empty( $post['fees'] ) ) $post['fees'] = nv_htmlspecialchars( $post['fees'] );
		if ( ! empty( $post['declarationform'] ) ) $post['declarationform'] = nv_htmlspecialchars( $post['declarationform'] );
		if ( ! empty( $post['legalbasis'] ) ) $post['legalbasis'] = nv_htmlspecialchars( $post['legalbasis'] );
