...
{
    global $global_config, $module_name, $lang_module, $module_info, $op, $nv_laws_listsigner, $nv_laws_setting;
    ...
    foreach ($array_data as $row) {
		...
		$row['signer'] = $nv_laws_listsigner[$row['sgid']]['title'];
        $xtpl->assign('ROW', $row);
...
function nv_theme_laws_detail($array_data, $other_cat = array(), $other_area = array(), $other_subject = array(), $other_signer = array())
{
	...
	if (!empty($array_data['bodytext'])) {
        $xtpl->parse('main.field.bodytext');
    }
