...
if ( ! nv_function_exists( 'nv_pso_block_newg' ) )
{
	function nv_block_config_new_pso( $module, $data_block, $lang_block )
	{
		$html = '';
        $html .= '<tr>';
	...
	function nv_block_config_new_pso_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
    ...
    function nv_pso_block_newg ( $block_config )
    {
        ...
        if ( ! empty( $numrow ) )
        {
            if ( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $modfile . '/block_new_pso.tpl' ) )
            {
            ...
            $xtpl = new XTemplate( 'block_new_pso.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $modfile );
...
if ( defined( 'NV_SYSTEM' ) )
{
    ...
    {
        $content = nv_pso_block_newg( $block_config );
    }
}