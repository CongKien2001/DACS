...
function viewcat_page_new($array_catpage, $array_cat_other, $generate_page)
{
    ...
    $xtpl->assign('LANG', $lang_module);
    $xtpl->assign('IMGWIDTH1', $module_config[$module_name]['homewidth']);
    $xtpl->assign('CATINFO', $global_array_cat[$catid]);
...
function detail_theme($news_contents, $array_keyword, $related_new_array, $related_array, $topic_array, $content_comment)
{
    ...
    $news_contents['hitstotal'] = number_format($news_contents['hitstotal'], 0, ',', '.');
    $news_contents['hitscm'] = number_format($news_contents['hitscm'], 0, ',', '.');
    ...
    if($news_contents['files']){
        foreach($news_contents['files'] as $key=>$file){
            $xtpl->assign('file_title', $file['title']);
            $xtpl->assign('file_url', $file['url']);
    ...
    if (! empty($related_new_array) or ! empty($related_array)) {
        if (! empty($related_new_array)) {
            ...
            $xtpl->parse('main.others.related_new_tab');
        }
        ...
        if (! empty($related_array)) {
            ...
            $xtpl->parse('main.others.related_tab');
        }
